        ...
        if not returned_requests.empty:
            ...
            for index, req in returned_requests.iterrows():
                with st.expander(f"Request ID: {req['id']} - Returned"):
                    ...
                    edit_title = st.text_input("Edit Title", value=req['title'], key=f"edit_title_{req['id']}")
                    ...
                    if st.button("Resubmit Request", key=f"resubmit_{req['id']}"):
                        ...
                        requests_df.loc[requests_df['id'] == req['id'], 'title'] = edit_title
                        ...
                        save_requests(requests_df)
                        log_request_history(req['id'], 'Edited', st.session_state['logged_in_user'], {'old_details': {'title': original_request['title'], 'description': original_request['description']}, 'new_details': {'title': edit_title, 'description': edit_description}})

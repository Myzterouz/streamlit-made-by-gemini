...
def create_request(user, title, description):
    ...
    log_request_history(new_id, 'Created', user, {'title': title, 'description': description})
...
if 'logged_in_user' in st.session_state:
    ...
    if st.session_state['user_role'] in ['user', 'approver', 'admin']:
        ...
        if not returned_requests.empty:
            ...
            for index, req in returned_requests.iterrows():
                with st.expander(f"Request ID: {req['id']} - Returned"):
                    ...
                    st.markdown(f"**Title:** {req['title']}") # Display title, not editable
        ...
        requests_df = load_requests()
        request_titles = requests_df['title'].unique().tolist()
        request_titles.insert(0, "<Select Request Title>")
        selected_title = st.selectbox("Select a Request Title to view history", request_titles)

        if selected_title != "<Select Request Title>":
            selected_request = requests_df[requests_df['title'] == selected_title].iloc[0] # Assuming title is unique for simplicity. In a real app, use ID.
            display_request_history(selected_request['id'])
